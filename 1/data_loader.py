...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # Convert to csv and store in the current directory (keeping original functionality)
        save_loc = "/var/lib/neo4j/import/" + file_path.split(".")[0] + '.csv'
        trips.to_csv(save_loc, index=False)

        # Load data into Neo4j (your implementation)
        with self.driver.session() as session:
            ...
            batch_size = 50000
            total_batches = (len(trips) // batch_size) + 1
            ...
            for i in range(0, len(trips), batch_size):
                ...
                MERGE (pickup:Location {name: toInteger(trip.PULocationID)})
                MERGE (dropoff:Location {name: toInteger(trip.DOLocationID)})
                CREATE (pickup)-[:TRIP {
                    distance: toFloat(trip.trip_distance),
                    fare: toFloat(trip.fare_amount),
